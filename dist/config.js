...
if (!postgresHost || !postgresDb || !postgresUser) {
    throw new Error("POSTGRES_HOST, POSTGRES_DB, and POSTGRES_USER are required (PostgreSQL from userver-datamgr)");
}
/**
 ...
 */
const resolvedPostgresPassRaw = process.env.POSTGRES_PASS?.trim() ||
    process.env.POSTGRES_PASSWORD?.trim() ||
    "";
const resolvedPostgresPass = resolvedPostgresPassRaw === "" ? undefined : resolvedPostgresPassRaw;
const appConfig = {
    ...
    userver: {
        ...
        configured: Boolean(userverAuthHost && userverSystemName && userverSystemToken),
    },
};
