package commands

import (
	"context"
	"net/http"

	"github.com/spf13/cobra"
	"go.uber.org/fx"
	"go.uber.org/fx/fxevent"

	"flora-hive/lib"
)

// Commands returns all CLI subcommands.
func Commands(opt fx.Option) []*cobra.Command {
	return []*cobra.Command{
		WrapServerCommand("app:serve", NewServeCommand(), opt),
		WrapSubCommand("migrate:up", NewMigrateUpCommand(), opt),
		WrapSubCommand("migrate:down", NewMigrateDownCommand(), opt),
	}
}

// WrapSubCommand runs a one-shot fx app (migrate).
func WrapSubCommand(name string, cmd lib.Command, opt fx.Option) *cobra.Command {
	wrappedCmd := &cobra.Command{
		Use:   name,
		Short: cmd.Short(),
		Run: func(c *cobra.Command, args []string) {
			logger := lib.GetLogger()
			opts := fx.Options(
				fx.WithLogger(func() fxevent.Logger {
					return logger.GetFxLogger()
				}),
				fx.Invoke(cmd.Run()),
			)
			ctx := context.Background()
			app := fx.New(opt, opts)
			err := app.Start(ctx)
			defer func(app *fx.App, ctx context.Context) {
				if internalErr := app.Stop(ctx); internalErr != nil {
					logger.Errorw("could not gracefully stop application", "err", internalErr)
				}
			}(app, ctx)
			if err != nil {
				logger.Fatal(err)
			}
		},
	}
	cmd.Setup(wrappedCmd)
	return wrappedCmd
}

// WrapServerCommand runs the long-lived HTTP server.
func WrapServerCommand(name string, cmd lib.Command, opt fx.Option) *cobra.Command {
	wrappedCmd := &cobra.Command{
		Use:   name,
		Short: cmd.Short(),
		Run: func(c *cobra.Command, args []string) {
			logger := lib.GetLogger()
			opts := fx.Options(
				opt,
				fx.WithLogger(func() fxevent.Logger {
					return logger.GetFxLogger()
				}),
				fx.Provide(cmd.Run()),
				fx.Invoke(func(*http.Server) {}),
			)
			fx.New(opts).Run()
		},
	}
	cmd.Setup(wrappedCmd)
	return wrappedCmd
}
