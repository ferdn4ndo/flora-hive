...
export async function listEnvironmentsForUser(userId: string) {
  const db = getDatabase();
  return db
...
export async function getMembership(environmentId: string, userId: string) {
  const db = getDatabase();
  const rows = await db
    .select()
    .from(environmentMembers)
    .where(
      and(
        eq(environmentMembers.environmentId, environmentId),
        eq(environmentMembers.userId, userId)
      )
    )
    .limit(1);
  return rows[0] ?? null;
...
export async function listMembers(environmentId: string) {
  const db = getDatabase();
  return db
    .select({
...
export async function removeMember(environmentId: string, userId: string) {
  const db = getDatabase();
  await db
...
export async function getHiveUserById(userId: string) {
  const db = getDatabase();
  const rows = await db
    .select()
    .from(hiveUsers)
