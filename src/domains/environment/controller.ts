...
function handleErr(res: Response, next: NextFunction, e: unknown) {
...
export function listEnvironments(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      res.json({
        environments: rows.map((r) =>
          environmentPublic(r.env, r.role)
        ),
      });
    } catch (e) {
...
export function postEnvironment(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      if (!req.auth) return;
      if (req.auth.kind === "api_key") {
...
export function getEnvironment(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
...
export function patchEnvironment(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
...
export function destroyEnvironment(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
      if (req.auth?.kind !== "jwt") {
...
export function listEnvironmentMembers(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
      if (req.auth?.kind === "api_key") {
...
export function postEnvironmentMember(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
      if (req.auth?.kind !== "jwt") {
      ...
      if (!target) {
        res.status(404).json({
          error: "not_found",
          message:
            "Hive user not found for that auth UUID; the user must call GET /v1/auth/me once after registering.",
        });
        return;
      }
      await upsertMember({
...
export function patchEnvironmentMember(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
...
export function deleteEnvironmentMember(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
      const userId = pathParam(req.params.userId);
