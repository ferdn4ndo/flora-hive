...
import {
  createDevice,
  deleteDeviceByEnvAndDeviceId,
  getDeviceByEnvAndDeviceId,
  getDeviceRowByEnvAndDeviceId,
  listDevicesByEnvironmentId,
  listDevicesInEnvironment,
  updateDeviceByEnvAndDeviceId,
} from "./services.js";
...
export function listDevices(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      ...
      if (req.auth.kind === "api_key") {
        ...
        const rows = await listDevicesByEnvironmentId(
          environmentId,
          parentDeviceId
        );
      ...
      const rows = await listDevicesInEnvironment(
        environmentId,
        req.auth.hiveUserId,
        parentDeviceId
      );
...
export function postDevice(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
...
export function getDeviceById(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      const environmentId = pathParam(req.params.environmentId);
      ...
      if (!req.auth) return;
      if (req.auth.kind === "api_key") {
        const row = await getDeviceRowByEnvAndDeviceId(
          environmentId,
          logicalDeviceId
        );
        if (!row) {
      ...
      const row = await getDeviceByEnvAndDeviceId(
        environmentId,
        logicalDeviceId,
        req.auth.hiveUserId
      );
...
export function patchDevice(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      if (req.auth?.kind !== "jwt") {
        res.status(403).json({ error: "forbidden", message: "JWT required" });
      ...
      const logicalDeviceId = pathParam(req.params.deviceId);
      const { deviceType, deviceId, displayName, parentDeviceId } = req.body || {};
      const row = await updateDeviceByEnvAndDeviceId(
        environmentId,
        logicalDeviceId,
        req.auth.hiveUserId,
        {
...
export function destroyDevice(req: Request, res: Response, next: NextFunction) {
  requireAuth(req, res, async () => {
    try {
      if (req.auth?.kind !== "jwt") {
        res.status(403).json({ error: "forbidden", message: "JWT required" });
      ...
      const environmentId = pathParam(req.params.environmentId);
      const logicalDeviceId = pathParam(req.params.deviceId);
      const ok = await deleteDeviceByEnvAndDeviceId(
        environmentId,
        logicalDeviceId,
        req.auth.hiveUserId
      );
