 ...
 */
export async function ensureUserverAuthBootstrap(): Promise<void> {
  ...
  if (!systemName || !systemToken) {
    console.log(
      "bootstrap: skipping userver-auth (USERVER_AUTH_SYSTEM_NAME or USERVER_AUTH_SYSTEM_TOKEN unset)"
    );
    return;
  }
  ...
  if (!authUser || authPassword === undefined || authPassword === "") {
    console.log(
      "bootstrap: skipping userver-auth bootstrap (USERVER_AUTH_USER / USERVER_AUTH_PASSWORD unset)"
    );
    return;
  }

  console.log("bootstrap: checking userver-auth (login probe)...");
  let loginStatus = await postLogin(
    host,
    systemName,
    systemToken,
    authUser,
  ...
  if (loginStatus === 200) {
    console.log(
      "bootstrap: userver-auth credentials OK; skipping system/register."
    );
    return;
  }
  ...
  if (!creationToken) {
    throw new Error(
      `bootstrap: login failed (HTTP ${loginStatus}) and USERVER_AUTH_SYSTEM_CREATION_TOKEN is empty; cannot create system.`
    );
  }

  console.log(
    `bootstrap: userver-auth login returned HTTP ${loginStatus} (expected when bootstrapping).`
  );
  ...
  if (sysRes.status !== 201 && sysRes.status !== 409) {
    throw new Error(
      `bootstrap: POST /auth/system failed HTTP ${sysRes.status}: ${sysBody}`
    );
  }
  console.log(
    sysRes.status === 201
  ...
  if (regRes.status !== 201 && regRes.status !== 409) {
    throw new Error(
      `bootstrap: POST /auth/register failed HTTP ${regRes.status}: ${regBody}`
    );
  }
  console.log(
    regRes.status === 201
  ...
  if (loginStatus !== 200) {
    throw new Error(
      `bootstrap: login still failing with HTTP ${loginStatus} after bootstrap`
    );
  }
