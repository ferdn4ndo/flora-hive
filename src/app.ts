...
import {
  ...
  publishMqtt,
} from "./domains/mqtt/services.js";
import {
  ForbiddenError,
  listEnvironmentsForUser,
  requireEnvAccess,
} from "./domains/environment/services.js";
import {
  getDeviceRowById,
  listDeviceRowIdsForEnvironments,
} from "./domains/device/services.js";
import { requireAuth } from "./domains/auth/middleware.js";

async function allowedDeviceRowIdsForRequest(
  req: Request
): Promise<string[] | null> {
  if (!req.auth) return [];
  if (req.auth.kind === "api_key") return null;
...
async function assertMqttPublishAllowed(req: Request, topic: string) {
  if (req.auth?.kind === "api_key") return;
  if (req.auth?.kind !== "jwt") {
    throw new ForbiddenError("Authentication required");
  }
  const normalized = normalizeTopic(topic, config.topicPrefix);
  const deviceRowId = parseDeviceRowIdFromTopic(
    normalized,
    config.topicPrefix
  );
  if (!deviceRowId) {
...
export function createApp() {
  ...
  app.get(
    ...
    envController.getEnvironment
  );
  app.patch(
    "/v1/environments/:environmentId",
    envController.patchEnvironment
  );
  app.delete(
    "/v1/environments/:environmentId",
    envController.destroyEnvironment
  );

  app.get(
    "/v1/environments/:environmentId/members",
    envController.listEnvironmentMembers
  );
  app.post(
    "/v1/environments/:environmentId/members",
    envController.postEnvironmentMember
  );
  app.patch(
    "/v1/environments/:environmentId/members/:userId",
    envController.patchEnvironmentMember
  );
  app.delete(
    "/v1/environments/:environmentId/members/:userId",
    envController.deleteEnvironmentMember
  );
  ...
  app.patch(
    "/v1/environments/:environmentId/devices/:deviceId",
    deviceController.patchDevice
  );
  app.delete(
    "/v1/environments/:environmentId/devices/:deviceId",
    deviceController.destroyDevice
  );
